...
LEFT JOIN barang br ON bd.barang_id = br.barang_id WHERE bd.id_gudang = 'B' AND bd.satuan = 'MC' AND b.tgl_trans BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."'";
...
<h3 align="center">STOK Global PT. Mina Jaya Lestari - Gudang B - <?= $nd->format('d/F/Y') ?></h3>
	...
	foreach(array_unique($arrb) as $brgkode){//foreach1
		...
		WHERE b.tgl_trans BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."' AND bd.id_gudang = 'B' AND bd.satuan = 'MC' AND br.kode = '$brgkode'";
		//by area
		$qsqlareasupp = mysql_query($sqlareasupp);
		...
		LEFT JOIN barang br ON bd.barang_id = br.barang_id
		LEFT JOIN supplier s ON b.supplier_nama = s.supplier_id 
		WHERE b.tgl_trans BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."' AND bd.satuan = 'MC' AND bd.id_gudang = 'B' AND br.kode = '$brgkode' GROUP BY barang_idx, br.barang_kategori";
		// echo $sql;
...
<b>KG</b>
	...
	$sqlbrg = "SELECT DISTINCT br.kode FROM beli b 
	LEFT JOIN beli_detail bd ON b.beli_id = bd.beli_id 
	LEFT JOIN barang br ON bd.barang_id = br.barang_id WHERE bd.id_gudang = 'B' AND bd.satuan = 'KG' AND b.tgl_trans BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."'";

	$qsqlbrg = mysql_query($sqlbrg);
	while($rsqlbrg = fetch(query($conn, $sqlbrg))){
		$arrb[] = $rsqlbrg['kode'];//array barang
	}
	...
	foreach(array_unique($arrb) as $brgkode){//foreach1
		...
		LEFT JOIN barang br ON bd.barang_id = br.barang_id
		LEFT JOIN supplier s ON b.supplier_nama = s.supplier_id 
		WHERE b.tgl_trans BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."' AND bd.id_gudang = 'B' AND bd.satuan = 'KG'  AND br.kode = '$brgkode'";
		//by area
		...
		LEFT JOIN barang br ON bd.barang_id = br.barang_id
		LEFT JOIN supplier s ON b.supplier_nama = s.supplier_id 
		WHERE b.tgl_trans BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."' AND bd.satuan = 'KG' AND bd.id_gudang = 'B' AND br.kode = '$brgkode' GROUP BY barang_idx, br.barang_kategori";
		// echo $sql;
...
FROM beli b 
LEFT JOIN beli_detail bd ON b.beli_id = bd.beli_id 
WHERE b.tgl_trans BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."' AND bd.id_gudang = 'B' GROUP BY bd.satuan ORDER BY bd.satuan DESC";
...
$counting2 = fetch(query($conn, $sql2));
