...
for($i =1; $i <= $dt; $i++){
	...
	LEFT JOIN barang br ON bd.barang_id = br.barang_id WHERE bd.id_gudang = 'B' AND b.tgl_trans BETWEEN '".$mt."-01' AND '".$mt.'-'.sprintf('%02d',$i)."'";
	...
	// $sqlareasupp = "SELECT DISTINCT s.kode_daerah FROM beli b LEFT JOIN supplier s ON b.supplier_nama = s.supplier_id WHERE b.tgl_trans BETWEEN '".$mt."-01' AND '".$mt.'-'.sprintf('%02d',$i)."'";
	// //by area
	// $qsqlareasupp = mysql_query($sqlareasupp);
	// while ($ds = mysql_fetch_array($qsqlareasupp)) {
	...
	?>
	<h3 align="center">STOK Global PT. Mina Jaya Lestari - Gudang B - <?= date('d/F/Y', strtotime($mt.'-'.$i)) ?></h3>
	<div style="float: left; width: 50%">
		...
		foreach(array_unique($arrb) as $brgkode){//foreach1
			...
			WHERE b.tgl_trans BETWEEN '".$mt."-01' AND '".$mt.'-'.sprintf('%02d',$i)."' AND bd.id_gudang = 'B' AND br.kode = '$brgkode'";
			//by area
			...
			LEFT JOIN barang br ON bd.barang_id = br.barang_id
			LEFT JOIN supplier s ON b.supplier_nama = s.supplier_id 
			WHERE b.tgl_trans BETWEEN '".$mt."-01' AND '".$mt.'-'.sprintf('%02d',$i)."' AND bd.id_gudang = 'B' AND br.kode = '$brgkode' GROUP BY bd.barang_id, s.kode_daerah, br.barang_kategori, bd.satuan";
			// echo $sql;
			...
			// var_dump(mysql_fetch_array($qsql2));
			?>
		</div>
	...
	FROM beli b 
	LEFT JOIN beli_detail bd ON b.beli_id = bd.beli_id 
	WHERE b.tgl_trans BETWEEN '".$mt."-01' AND '".$mt.'-'.sprintf('%02d',$i)."' AND bd.id_gudang = 'B' GROUP BY bd.satuan ORDER BY bd.satuan DESC";
	...
	WHERE j.tgl_jual BETWEEN '".$mt."-01' AND '".$mt.'-'.sprintf('%02d',$i)."' AND jd.id_gudang = 'B' GROUP BY jd.satuan";
	
	$counting2 = nrow(query($sql2));

	if($counting > 0){
		while($d = fetch(query($conn, $sql2))){
