...
<?php
//get lowest date
$sqlmindt = "SELECT MIN(tgl_trans) AS lodate FROM beli";
...
LEFT JOIN barang br ON bd.barang_id = br.barang_id WHERE bd.id_gudang = 'A' AND bd.satuan = 'MC' AND b.tgl_trans BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."'";

while($rsqlbrg = fetch(query($conn, $sqlbrg))){
	$arrb[] = $rsqlbrg['kode'];//array barang
}
?>
<h3 align="center">STOK Global PT. Mina Jaya Lestari - Gudang A - <?= $nd->format('d/F/Y') ?></h3>
	<b>MC</b>
	<?php
	if(count($arrb) > 0){
	$i = 0;
	foreach(array_unique($arrb) as $brgkode){//foreach1
		...
		WHERE b.tgl_trans BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."' AND bd.id_gudang = 'A' AND bd.satuan = 'MC' AND br.kode = '$brgkode'";
		//by area
		...
		LEFT JOIN barang br ON bd.barang_id = br.barang_id
		LEFT JOIN supplier s ON b.supplier_nama = s.supplier_id 
		WHERE b.tgl_trans BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."' AND bd.satuan = 'MC' AND bd.id_gudang = 'A' AND br.kode = '$brgkode' GROUP BY barang_idx, br.barang_kategori";
		// echo $sql;
...
<b>KG</b>
	...
	$sqlbrg = "SELECT DISTINCT br.kode FROM beli b 
	LEFT JOIN beli_detail bd ON b.beli_id = bd.beli_id 
	LEFT JOIN barang br ON bd.barang_id = br.barang_id WHERE bd.id_gudang = 'A' AND bd.satuan = 'KG' AND b.tgl_trans BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."'";
	...
	foreach(array_unique($arrb) as $brgkode){//foreach1
		...
		LEFT JOIN barang br ON bd.barang_id = br.barang_id
		LEFT JOIN supplier s ON b.supplier_nama = s.supplier_id 
		WHERE b.tgl_trans BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."' AND bd.id_gudang = 'A' AND bd.satuan = 'KG'  AND br.kode = '$brgkode'";
		//by area
		...
		LEFT JOIN barang br ON bd.barang_id = br.barang_id
		LEFT JOIN supplier s ON b.supplier_nama = s.supplier_id 
		WHERE b.tgl_trans BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."' AND bd.satuan = 'KG' AND bd.id_gudang = 'A' AND br.kode = '$brgkode' GROUP BY barang_idx, br.barang_kategori";
		// echo $sql;
		$counting = nrow(query($qsql));
...
FROM beli b 
LEFT JOIN beli_detail bd ON b.beli_id = bd.beli_id 
WHERE b.tgl_trans BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."' AND bd.id_gudang = 'A' GROUP BY bd.satuan ORDER BY bd.satuan DESC";
...
WHERE j.tgl_jual BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."' AND jd.id_gudang = 'A' GROUP BY jd.satuan";

$counting2 = nrow(query($qsql2));
