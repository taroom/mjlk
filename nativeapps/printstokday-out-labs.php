...
LEFT JOIN barang br ON bd.barang_id = br.barang_id WHERE bd.id_gudang = 'A' AND bd.satuan = 'MC' AND b.tgl_jual BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."'";
// echo "$sqlbrg";
...
<h3 align="center">STOK Global PT. Mina Jaya Lestari - Gudang A - <?= $nd->format('d/F/Y') ?></h3>
	...
	foreach(array_unique($arrb) as $brgkode){//foreach1
		...
		WHERE b.tgl_jual BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."' AND bd.id_gudang = 'A' AND bd.satuan = 'MC' AND br.kode = '$brgkode'";
		//by area
		// echo $sqlareasupp;
		...
		LEFT JOIN barang br ON bd.barang_id = br.barang_id
		LEFT JOIN pelanggan s ON b.pelanggan_nama = s.pelanggan_id 
		WHERE b.tgl_jual BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."' AND bd.satuan = 'MC' AND bd.id_gudang = 'A' AND br.kode = '$brgkode' GROUP BY barang_idx, br.barang_kategori";
		// echo $sql;
...
<b>KG</b>
	...
	$sqlbrg = "SELECT DISTINCT br.kode FROM jual b 
	LEFT JOIN jual_detail bd ON b.jual_id = bd.jual_id 
	LEFT JOIN barang br ON bd.barang_id = br.barang_id WHERE bd.id_gudang = 'A' AND bd.satuan = 'KG' AND b.tgl_jual BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."'";
	...
	foreach(array_unique($arrb) as $brgkode){//foreach1
		...
		LEFT JOIN barang br ON bd.barang_id = br.barang_id
		LEFT JOIN pelanggan s ON b.pelanggan_nama = s.pelanggan_id 
		WHERE b.tgl_jual BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."' AND bd.id_gudang = 'A' AND bd.satuan = 'KG'  AND br.kode = '$brgkode'";
		//by area
		...
		LEFT JOIN barang br ON bd.barang_id = br.barang_id
		LEFT JOIN pelanggan s ON b.pelanggan_nama = s.pelanggan_id 
		WHERE b.tgl_jual BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."' AND bd.satuan = 'KG' AND bd.id_gudang = 'A' AND br.kode = '$brgkode' GROUP BY barang_idx, br.barang_kategori";
		// echo $sql;
...
FROM jual b 
LEFT JOIN jual_detail bd ON b.jual_id = bd.jual_id 
WHERE b.tgl_jual BETWEEN '".$dsqlmindt['lodate']."' AND '".$nd->format('Y-m-d')."' AND bd.id_gudang = 'A' GROUP BY bd.satuan ORDER BY bd.satuan DESC";
